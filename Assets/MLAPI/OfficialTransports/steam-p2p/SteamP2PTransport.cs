    ...
    public class SteamP2PTransport : Transport
    {
        ...
        public override ulong GetCurrentRtt(ulong clientId)
        {
            if (isServer)
            {
                ...
                {
                    if (NetworkingManager.Singleton.LogLevel <= LogLevel.Error) NetworkLog.LogWarningServer("SteamP2PTransport - Can't GetCurrentRtt from client, client not connected, clientId: " + clientId);
                }
            }
            else
            {
        ...
        public override void Init()
        {
            ...
            {
                if (NetworkingManager.Singleton.LogLevel <= LogLevel.Error) NetworkLog.LogWarningServer("SteamP2PTransport - Init - Steamworks.NET SteamManager.Initialized not found, SteamP2PTransport can not run without it");
                return;
            }
            ...
            if (!propertyValue)
            {
                if (NetworkingManager.Singleton.LogLevel <= LogLevel.Error) NetworkLog.LogWarningServer("SteamP2PTransport - Init - Steamworks.NET is not Initialized, SteamP2PTransport can not run without it");
                return;
            }
        ...
        public override void Send(ulong clientId, ArraySegment<byte> data, string channelName)
        {
            ...
            {
                if (NetworkingManager.Singleton.LogLevel <= LogLevel.Error) NetworkLog.LogWarningServer("SteamP2PTransport - Can't Send to client, channel with channelName: " + channelName + " is not present");
                return;
            }
            ...
            else
            {
                ...
                {
                    if (NetworkingManager.Singleton.LogLevel <= LogLevel.Error) NetworkLog.LogWarningServer("SteamP2PTransport - Can't Send to client, client not connected, clientId: " + clientId);
                }
            }
        }
